...
var getRowData = function getRowData(list, row, fieldName, _default) {
  var item = getItem(list, row);
...
var formatDHM = function formatDHM(value) {
  var M = Math.trunc(value / 43200);
  var Y = Math.trunc(M / 12);
  var d = Math.trunc((value - M * 43200) / 1440);
  var h = Math.trunc((value - M * 43200 - d * 1440) / 60);
  var m = Math.trunc(value - M * 43200 - d * 1440 - h * 60);
  M = Math.trunc(M - Y * 12);
